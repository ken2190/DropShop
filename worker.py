...
class Worker(threading.Thread):
    ...
    def __category_menu(self):
        # user menu to get the categories of all products as buttons 
        ...
        # Add the sorted categories to the list
        for category in category_list:
            keyboard_buttons.append([category])

        # Create the buttons
        keyboard = telegram.ReplyKeyboardMarkup(keyboard_buttons, one_time_keyboard=True)

        # Load the saved category from the instance variable
        if self.chosen_category:
            # Filter the products by the saved category
            products = self.session.query(db.Product).filter_by(category=self.chosen_category, deleted=False).all()

        # Show the category menu until the user makes a choice
        while True:
            # Send the keyboard
            self.bot.send_message(self.chat.id, self.loc.get("menu_category"), reply_markup=keyboard)
            # Wait for a reply
            reply = self.__wait_for_regex(r"(.*)", cancellable=True)
            # Propagate CancelSignals
            if isinstance(reply, CancelSignal):
                return reply
            # Find the products in the database that belong to the selected category
            products = self.session.query(db.Product).filter_by(category=reply, deleted=False).all()
            # Ensure the category exists
            if not products:
                self.bot.send_message(self.chat.id, self.loc.get("error_category_does_not_exist"))
                continue
            
            # Save the selected category to the instance variable
            self.chosen_category = reply

            # Call the next method and pass the selected category
            return self.__region_menu(self.chosen_category, products)



    def __region_menu(self, chosen_category, products):
        # debug
        print(f"Selected category: {chosen_category}")
        print(f"Products: {products}")
        # user menu to get the regions of all remaining products after category selection as buttons    
        log.debug("Displaying __region_menu")
        # Create a list containing all the keyboard button strings
        keyboard_buttons = [[self.loc.get("menu_cancel")]]
        # Add to the list all the regions
        regions = set()
        for product in products:
            regions.add(product.region)
        region_list = sorted(list(regions))
        for region in region_list:
            keyboard_buttons.append([region])

        # Create the buttons
        keyboard = telegram.ReplyKeyboardMarkup(keyboard_buttons, one_time_keyboard=True)

        # Show region choices until user makes a choice
        while True:
            # Send the keyboard
            self.bot.send_message(self.chat.id, self.loc.get("menu_region"), reply_markup=keyboard)
            # Wait for a reply
            reply = self.__wait_for_regex(r"(.*)", cancellable=True)
            # Propagate CancelSignals
            if isinstance(reply, CancelSignal):
            ...
            products_filtered_by_region = self.session.query(db.Product).filter_by(region=reply, deleted=False).all()
            # Ensure the region exists
            if not products_filtered_by_region:
                self.bot.send_message(self.chat.id, self.loc.get("error_region_does_not_exist"))
                continue
            
            # Save the selected region to the instance variable
            self.chosen_region = reply

            # Call the next method and pass the selected region
            return self.__final_product_list(chosen_category, self.chosen_region)


        

    def __final_product_list(self, chosen_category, chosen_region):
        products = self.session.query(db.Product).filter_by(category=chosen_category, region=chosen_region).all()
        #show filtered product list and get the product id and buy selected product
        log.debug("Displaying __final_product_list")
        # Create a list containing all the keyboard button strings
        keyboard_buttons = [[self.loc.get("menu_cancel")]]
        # Add to the list all the products
        for product in products:
            keyboard_buttons.append([product.name])
        # Create the buttons    
        keyboard = telegram.ReplyKeyboardMarkup(keyboard_buttons, one_time_keyboard=True)
        # Show product choices until user makes a choice
        while True:
            # Send the keyboard
            self.bot.send_message(self.chat.id, self.loc.get("menu_packagesize"), reply_markup=keyboard)
            # Wait for a reply
            reply = self.__wait_for_regex(r"(.*)", cancellable=True)
            # Propagate CancelSignals
            if isinstance(reply, CancelSignal):
                return reply
            # Find the product in the database
            product = self.session.query(db.Product).filter_by(name=reply).one_or_none()
            # Ensure the product exists
            if not product:
                self.bot.send_message(self.chat.id, self.loc.get("error_product_does_not_exist"))
                continue
            
            # Save the selected product to the instance variable
            self.chosen_product = product

            # Call the next method and pass the selected product
            return self.__buy_product(self.chosen_product)
    
    # purchases the selected product, deducts price from user credit and deletes the product from the database
    def __buy_product(self, chosen_product):
        log.debug("Displaying __buy_product")
    
        # check if user has enough credit to buy the product
        if self.user.credit < chosen_product.price:
            self.bot.send_message(self.chat.id, self.loc.get("error_not_enough_credit"))
            return self.__add_credit_menu()
        
        # deduct the price of the product from the user's credit
        self.user.credit -= chosen_product.price
        
        # get the product's region and category
        product_region = chosen_product.region
        product_category = chosen_product.category
        
        # create the caption for the photo check for language  
        if self.user.language == "en":
            caption = f"✅Thanks for purchasing, here is the drop location for your {product_category} in {product_region}:"
            caption += f"Which you can find at coordinates:\n🧭 {chosen_product.description} 🧭"

        elif self.user.language == "ee":
            caption = f"✅Tänan ostmast, siin on teie asukoht tootele {product_category} linnaosas {product_region}:"
            caption += f"Toote leidmiseks täpsed koordinaadid on:\n🧭 {chosen_product.description} 🧭"
        elif self.user.language == "ru":
            caption = f"✅Спасибо за покупку, вот место доставки вашего {product_category} в {product_region}:"
            caption += f"Который вы можете найти по координатам:\n🧭 {chosen_product.description} 🧭"    
        else:
            caption = f"✅Thanks for purchasing, here is the drop location for your {product_category} in {product_region}:"
            caption += f"Which you can find at coordinates:\n🧭 {chosen_product.description} 🧭"
        ...
        self.session.delete(chosen_product)
        # commit the changes to the database
        self.session.commit()
        
        # send user the product image and description
        self.bot.send_photo(self.chat.id, chosen_product.image, caption=caption)
    ...
    def __add_credit_menu(self):
        ...
        if self.cfg["Bitcoin"]["api_key"] != "":
            ...
            keyboard.append([telegram.KeyboardButton("LITECOIN")])
        ...
        # Wait for a reply from the user
        selection = self.__wait_for_specific_message(
            [self.loc.get("menu_cash"), self.loc.get("menu_credit_card"), "🛡 Bitcoin", "LITECOIN", self.loc.get("menu_cancel")],
            cancellable=True)
        ...
        # If the user has selected the Bitcoin option...
            ...
            # Go to the pay with litecoin function
            self.__add_credit_ltc()
    ...
    def __add_credit_ltc(self):
        log.debug("Displaying __add_credit_ltc")
        ...
        # If the user cancelled the action...
            ...
            # Exit the function
            return
        # The amount is valid, fetch ltc amount and address
        url = 'https://api.coinbase.com/v2/prices/LTC-EUR/spot'
        response = requests.get(url)
        data = response.json()

        ltc_price = float(data['data']['amount'])
        print(f'Current LTC price: ${ltc_price:.2f}')
        ltc_price = Blockonomics.fetch_new_btc_price() * 0.004
        ltc_amount = float(raw_value) / ltc_price
             # Send a message containing the btc pay info
                # Send a message containing the btc pay info
        self.bot.send_message_markdown(self.chat.id, "To pay, send this amount:\n`" 
                                                    + str(ltc_amount) 
                                                    + "`\nto this litecoin address:\n`" 
                                                    + str("LaUNMPWNQ5uSvHWqxKnK6tinuPg3gCTVHa") + "`")
    ...
    #    product_names.insert(3, self.loc.get("menu_category"))
    #    product_names.insert(4, self.loc.get("menu_region"))
    ...
    def __edit_product_menu(self, product: Optional[db.Product] = None):
        ...
        self.bot.send_message(self.chat.id, self.loc.get("ask_product_category"))
                       # Display the current category if you're editing an existing product  
        if product:
            self.bot.send_message(self.chat.id, self.loc.get("edit_current_value", value=escape(product.category)),
                                  reply_markup=cancel)
            # Wait for an answer
        category = self.__wait_for_regex(r"(.*)", cancellable=bool(product))              
        self.bot.send_message(self.chat.id, self.loc.get("ask_product_region"))
        # Display the current regionif you're editing an existing product
        if product:
            self.bot.send_message(self.chat.id, self.loc.get("edit_current_value", value=escape(product.region)),
                              reply_markup=cancel)
        # Wait for an answer
        region = self.__wait_for_regex(r"(.*)", cancellable=bool(product))

            

        self.bot.send_message(self.chat.id, self.loc.get("ask_product_packagesize"))
        # Display the current packagesize if you're editing an existing product
        if product:
            self.bot.send_message(self.chat.id, self.loc.get("edit_current_value", value=escape(product.packagesize)),
                                  reply_markup=cancel)
        # Wait for an answer
        packagesize = int(self.__wait_for_regex(r"(.*)", cancellable=bool(product)))
        ...
        # Wait for an answer
        photo_list = self.__wait_for_photo(cancellable=False)
    ...
    def __delete_product_menu(self):
        ...
        if isinstance(selection, CancelSignal):
            ...
            # Find the selected product
            product = self.session.query(db.Product).filter_by(name=selection, deleted=False).first()
    ...
    def __language_menu(self):
        ...
        if "ee" in self.cfg["Language"]["enabled_languages"]:
            lang = "🇪🇪 Eesti"
